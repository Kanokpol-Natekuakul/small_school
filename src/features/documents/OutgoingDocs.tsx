...
const docSchema = zod.object({
  doc_no: zod.string().min(1, 'กรุณากรอกเลขที่หนังสือส่ง'),
  ...
  receiver: zod.string().min(1, 'กรุณากรอกชื่อผู้รับ/หน่วยงานปลายทาง'),
...
export const OutgoingDocs: React.FC<OutgoingDocsProps> = ({ currentUser }) => {
  ...
  useEffect(() => {
    ...
    if (searchTerm) {
      result = result.filter(d => 
        ...
        d.receiver.toLowerCase().includes(searchTerm.toLowerCase())
      );
    }
  ...
  const handleDelete = async (id: string) => {
    if (confirm('คุณต้องการลบทะเบียนหนังสือส่งเล่มนี้ใช่หรือไม่?')) {
      try {
  ...
  const handleExport = () => {
    exportDocumentsToExcel(filteredDocs, 'ทะเบียนหนังสือส่ง_SmartSchool');
  };
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h2 className="text-slate-800 text-xl font-bold flex items-center gap-2">
            ...
            ระบบทะเบียนหนังสือส่งออก (Outgoing Documents)
          </h2>
          <p className="text-slate-500 text-xs mt-1">บริหารจัดการ ลงทะเบียน ออกเลขแฟ้มหนังสือ และส่งหนังสือราชการภายนอก</p>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          {(currentUser.role === 'admin' || currentUser.role === 'general_staff' || currentUser.role === 'registrar') && (
            <button
      ...
      <div className="p-4 bg-white border border-slate-200 rounded-2xl shadow-xs grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <div className="relative md:col-span-2">
          ...
          <input
            type="text"
            placeholder="ค้นหาตาม เลขที่ส่ง, เรื่อง หรือหน่วยงานปลายทาง..."
            value={searchTerm}
        ...
        <div className="flex items-center gap-2">
          ...
          >
            ...
            <option value="archived">ส่งออกแล้ว</option>
      ...
      <div className="grid grid-cols-1 gap-4 md:hidden">
        {isLoading ? (
          ...
          <div className="py-16 text-center bg-white border border-slate-200 rounded-2xl shadow-xs">
            ...
            <p className="text-slate-500 font-medium">ไม่พบเอกสารส่งตามเงื่อนไขที่กรอง</p>
          </div>
        )}
      </div>
      ...
      <div className="hidden md:block bg-white border border-slate-200 rounded-2xl shadow-xs overflow-hidden">
        {isLoading ? (
          ...
          <div className="overflow-x-auto">
            <table className="w-full border-collapse text-left text-xs">
              <thead className="bg-slate-50 text-slate-500 font-bold border-b border-slate-200">
                <tr>
                  <th className="p-4 w-36">เลขที่ส่งหนังสือ</th>
                  <th className="p-4 w-28">วันที่ส่งออก</th>
                  <th className="p-4">เรื่อง</th>
                  <th className="p-4">ผู้รับ / หน่วยงานปลายทาง</th>
          ...
          <div className="py-16 text-center">
            <AlertCircle className="h-10 w-10 text-slate-300 mx-auto mb-2" />
            <p className="text-slate-500 font-medium">ไม่พบเอกสารส่งตามเงื่อนไขที่กรอง</p>
            <p className="text-slate-400 text-[10px] mt-1 font-light">สามารถกด "ออกเลขหนังสือส่ง" เพื่อออกสัญลักษณ์จดหมายราชการภายนอกได้</p>
          </div>
        )}
      </div>
          ...
          <div className="w-full max-w-lg bg-white border border-slate-200 rounded-3xl shadow-2xl relative flex flex-col max-h-[90vh]">
            <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100 shrink-0">
              <h3 className="font-bold text-slate-800 text-sm">
                {editingDoc ? '📝 แก้ไขเลขที่และหนังสือส่ง' : '📤 ออกเลขและลงทะเบียนหนังสือส่งออก'}
              </h3>
              <button
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="flex-1 overflow-y-auto p-6 space-y-4 text-xs">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block font-semibold text-slate-600 mb-1 pl-0.5">เลขทะเบียนส่ง *</label>
                  <input
                    type="text"
                    {...register('doc_no')}
                ...
                <div>
                  <label className="block font-semibold text-slate-600 mb-1 pl-0.5">วันที่ออกจดหมาย *</label>
                  <input
                    type="date"
                    {...register('doc_date')}
              ...
              <div>
                <label className="block font-semibold text-slate-600 mb-1 pl-0.5">เรื่อง (หัวข้อเอกสารส่ง) *</label>
                <input
                  type="text"
                  placeholder="เช่น เรื่องขอขอบคุณวิทยากรโครงการวิชาการ หรือ หนังสือเชิญร่วมการประชุม..."
                  {...register('title')}
                  className="w-full px-3.5 py-2.5 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-emerald-500/30 text-xs"
                />
              ...
              <div>
                <label className="block font-semibold text-slate-600 mb-1 pl-0.5">รายละเอียดเพิ่มเติม / วัตถุประสงค์</label>
                <textarea
                  rows={3}
                  placeholder="ข้อมูลส่งออกแบบย่อ หรือรายละเอียดประสานงานเพิ่มเติม..."
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block font-semibold text-slate-600 mb-1 pl-0.5">ผู้ส่ง *</label>
                  <input
                    type="text"
                    {...register('sender')}
                    className="w-full px-3.5 py-2.5 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-emerald-500/30 text-xs"
                ...
                <div>
                  <label className="block font-semibold text-slate-600 mb-1 pl-0.5">ผู้รับ (หน่วยงานปลายทาง) *</label>
                  <input
                    type="text"
                    placeholder="โรงเรียนปลายทาง หรือส่วนงานภายนอก"
                    {...register('receiver')}
                    className="w-full px-3.5 py-2.5 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-emerald-500/30 text-xs"
                  />
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block font-semibold text-slate-600 mb-1 pl-0.5">สถานะเอกสาร</label>
                  <select
                    {...register('status')}
                    className="w-full py-2.5 px-3 border border-slate-200 rounded-xl text-xs focus:outline-none focus:ring-2 focus:ring-emerald-500/30 cursor-pointer"
                  >
                    <option value="pending_approval">เสนอผู้อำนวยการลงนาม (Pending)</option>
                    <option value="approved">อนุมัติและลงนามแล้ว (Approved)</option>
                    <option value="archived">จัดส่งหนังสือเรียบร้อย (Archived)</option>
              ...
              <div className="pt-4 border-t border-slate-100 flex items-center justify-end gap-3 shrink-0 flex-row">
                ...
                >
                  {editingDoc ? 'บันทึกการแก้ไข' : 'ออกเลขส่ง'}
                </button>
              </div>
            </form>
          </div>
